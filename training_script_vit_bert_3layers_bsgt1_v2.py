...
class LightningModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.model(pixel_values=pixel_values, attention_mask=encoder_attention_mask, labels=labels, decoder_attention_mask=attention_mask).loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        # remove pad zeros during inference using dataloader
        n_imgs = int(torch.sum(encoder_attention_mask).cpu().item())

        pixel_values = pixel_values[:, :n_imgs,:]
        
        # labels, attention_mask = labels.squeeze(0), attention_mask.squeeze(0)
        caption_ids = self.model.generate(pixel_values, max_length=self.max_gen_len, num_beams=2, do_sample=True)
...
if __name__=='__main__':
    ...
    # data loaders
    train_loader = torch.utils.data.DataLoader(ResnetPlusVitDatasetV3(df_path,text_decode_model=decoder_model_name, dtype='train'), 
                                               batch_size=batch_size, shuffle=True, num_workers=num_workers)
    val_loader = torch.utils.data.DataLoader(ResnetPlusVitDatasetV3(df_path,text_decode_model=decoder_model_name, dtype='val'), 
    ...
    csv_logger = CSVLogger(save_dir="/scratch/ss4yd/logs_only_vit_bert_fullexpv2_freezeFalse/", name=f"my_model")
    ...
    trainer.fit(
        ...
        # ckpt_path ="/scratch/ss4yd/logs_only_vit_bert_fullexpv2_freezeFalse/my_model/version_19/checkpoints/epoch=2-val_loss=0.94-step=8502.00.ckpt"
    )
