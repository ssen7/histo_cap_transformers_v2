...
class ViTModelCustom(ViTPreTrainedModel):
    def __init__(self, config, path_input_dim=384,  size_arg = "small", dropout=0.25, pretrain_4k='None', freeze_4k=False, pretrain_WSI='None', freeze_WSI=False):
        ...
        self.global_transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=576, nhead=3, dim_feedforward=576, dropout=dropout, activation='relu'
            ), 
            num_layers=2
        )
    ...
    def forward(self, pixel_values, **kwargs):
        ...
        h_4096 = self.global_phi(pixel_values)
        h_4096 = self.global_transformer(h_4096)
        # A_4096, attn_inp = self.global_attn_pool(h_4096)  
        # print(A_4096.shape)
        # A_4096 = torch.transpose(A_4096, 2, 1)
        # print(A_4096.shape)
        # A_4096 = F.softmax(A_4096, dim=1) 
        # print(A_4096.shape, h_4096.shape)
        # h_4096_weighted = torch.bmm(A_4096, h_4096)
        # h_WSI = self.global_rho(torch.cat([h_4096_weighted, h_4096], dim=1))
        h_WSI = self.global_rho(h_4096)
        # print(h_WSI.shape)
